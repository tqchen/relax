...
void StructInfoVisitor::VisitStructInfo_(const FuncStructInfoNode* op) {
  for (StructInfo param : op->params) {
    this->VisitStructInfo(param);
  }

  if (op->ret.defined()) {
    this->VisitStructInfo(op->ret.value());
  }
}
...
StructInfo StructInfoMutator::VisitStructInfo_(const FuncStructInfoNode* op) {
  Array<StructInfo> params =
      op->params.Map([this](const StructInfo& sinfo) { return this->VisitStructInfo(sinfo); });

  Optional<StructInfo> ret;
  if (op->ret.defined()) {
    ret = this->VisitStructInfo(ret.value());
  }
  ...
  if (params.same_as(op->params) && ret.same_as(op->ret)) {
    ...
    return FuncStructInfo(params, ret.value(), op->span);
  }
}
