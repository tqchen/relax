...
class VirtualMachineImpl : public VirtualMachine {
 ...
 protected:
   ...
   */
  inline void PrepareFuncCache(Index func_index);
  /*!
 ...
 private:
   ...
   *        The value can be a VMClosure or a PackedFunc.
   ...
   */
  std::vector<ObjectRef> func_cache_;
  //--------------------------------------------------------
...
void VirtualMachineImpl::Init(const std::vector<Device>& devices,
  ...
  for (const auto& constant : exec_->constants) {
    if (constant.type_code() != kTVMNDArrayHandle) {
      ...
      this->constants.push_back(ConvertRegToDevice(constant, devices[0]));
    }
  }
  // setup function cache
  this->func_cache_.resize(exec_->func_table.size(), ObjectRef(nullptr));
}
...
void VirtualMachineImpl::PrepareFuncCache(Index func_index) {
  // fast path, function already in cache;
  ICHECK_LT(func_index, func_cache_.size());

  if (func_cache_[func_index].defined()) return;

  const VMFuncInfo& info = exec_->func_table[func_index];

  Optional<ObjectRef> func;
  if (info.kind == VMFuncInfo::FuncKind::kPackedFunc) {
    // lookup packed function and populate
    if (this->lib.defined()) {
      func = this->lib.value()->GetFunction(info.name, true);
    }
    if (!func.defined()) {
...
void VirtualMachineImpl::RunInstrCall(VMFrame* curr_frame, Instruction instr) {
  ...
  for (Index i = 0; i < instr.num_args; ++i) {
    ...
    switch (arg.kind()) {
      ...
      case Instruction::ArgKind::kFuncIdx: {
        this->PrepareFuncCache(arg.value());
        setter(i, this->func_cache_[arg.value()]);
        break;
      }
      default: {
  ...
  this->PrepareFuncCache(instr.func_idx);
  this->InvokeClosurePacked(func_cache_[instr.func_idx], args, &ret);
