...
class BuiltinLower : public StmtExprMutator {
 public:
  ...
  void SetPackedArg(PrimExpr arg, const Var& value_stack, const Buffer& tcode_stack,
    ...
    if (call_pattern && call_pattern->op.same_as(builtin::anylist_setitem_call_packed())) {
  ...
  PrimExpr MakeAnyListSetItemCallPacked(const CallNode* op) {
    PrimExpr list_handle = op->args[0];
    PrimExpr list_index = op->args[1];
    Call call = MakeCallPackedGeneric(op, 2, builtin::tvm_call_packed_lowered(), true);
