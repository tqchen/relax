 ...
 */
using StructInfoDeriveFunc = TypedEnvFunc<StructInfo(const Call& call)>;
...
class FuncStructInfoNode : public StructInfoNode {
 public:
   ...
   * \note ret should be supplied when derive_func is not provided.
   */
  Optional<StructInfo> ret;
  /*!
   * \brief Derive function of opaque functions that can take any number of parameters.
   * \note When derive_func is not empty, then params and ret are ignoed.
   */
  Optional<StructInfoDeriveFunc> derive_func;

  /*! \return Whether the func struct info is opque. */
  bool IsOpaque() const { return derive_func.defined(); }
...
class FuncStructInfo : public StructInfo {
 public:
  ...
  /*!
   * \brief Struct info for functions.
   *
   ...
   */
  TVM_DLL static FuncStructInfo OpaqueFunc(Optional<StructInfoDeriveFunc> derive_func = NullOpt,
                                           Span span = Span());
